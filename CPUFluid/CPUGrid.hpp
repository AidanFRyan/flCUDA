...
template <typename T>
CPUGrid<T>::CPUGrid(int x, int y, int z, double dx, double dt, double density) {
	this->x = x;
	this->y = y;
	this->z = z;
	printf("allocing %lu bytes\n", sizeof(CPUVoxel<T>)*x*y*z);
	this->a = std::move(CPUTriVec<T>(x, y, z, dx, dt, density));
...
template <typename T>
void CPUGrid<T>::setTstep(double t){
	this->dt = t;
	this->a.dt = t;
}

template <typename T>
void CPUGrid<T>::nullifyHostCPUTriVecs(){
	a.a = nullptr;
}

template <typename T>
void CPUGrid<T>::nullifyDeviceCPUTriVecs(){
}

template <typename T>
void CPUGrid<T>::printParts(string filename){
	ofstream f;
	f.open(filename);
	printf("%d particles\n", particles.size());
...
template <typename T>
CPUGrid<T>::CPUGrid(const CPUGrid<T>& in){
	this->x = in.x;
	this->y = in.y;
	this->z = in.z;
	this->dx = in.dx;
...
template <typename T>
const CPUGrid<T>& CPUGrid<T>::operator=(const CPUGrid<T>& in){
	a = in.a;
	x = in.x;	
	y = in.y;
	z = in.z;
...
template <typename T>
void CPUGrid<T>::print(){
	for(int k = z-1; k >= 0; k--){
		printf("\nZ = %d\n", k);
		for(int j = y-1; j >= 0; j--){
			for(int i = x-1; i >= 0; i--){
				CPUVoxel<T> *t = &a.get(i, j, k);
				printf("| %f |", t->p);
			}
			printf("\n");
		}
...
template <typename T>
void CPUGrid<T>::initializeParticles(){
...
template <typename T>
void CPUGrid<T>::interpU() {	//interpolate from particle velocities to grid
...
template <typename T>
void CPUGrid<T>::advectParticles(){
#pragma omp parallel for
	for (int p = 0; p < particles.size(); ++p) {
...
template <typename T>
void CPUGrid<T>::reinsertOOBParticles() {
#pragma omp parallel for
	for (int p = 0; p < particles.size(); ++p) {
		int i = floor(particles[p].p.x / dx), j = floor(particles[p].p.y / dx), k = floor(particles[p].p.z / dx);
