...
namespace Cu {
	...
	class Stencil {
		...
		int* s, size;
	public:
		__device__ __host__ bool contains(int l) {
			bool t = false;
			for (int i = 0; i < size; ++i) {//iterate entire list to avoid branch divergence
				if (s[i] == l) {
					t = true;
				}
			}
	...
	public:
		...
		__host__ void setSize(int tx, int ty, int tz) {
			x = tx;
			y = ty;
			z = tz;
			if (t)
		...
		__device__ __host__ bool contains(int tx, int ty, int tz) {
			return t[tx*ty].contains[tz];
		}
	};
