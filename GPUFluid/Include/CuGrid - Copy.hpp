...
void CuGrid<T>::initializeList(int n){
	...
	cudaMemcpy(d_list, list, sizeof(Particle<T>)*n, cudaMemcpyHostToDevice);
	numParticles = n;
	cudaDeviceSynchronize();
}
...
void CuGrid<T>::readParticlesFromTP(FlipFluidBasisThreadData* in, Vec3<T>*& v, Vec3<T>*& p, const Vec3<T>& offsetP) {	//currently set up so it doesn't know it can keep a copy on GPU at all times
	...
	if (list == nullptr) {
		list = new Particle<T>[in->datas.Count()];
	}
	if (this->d_list == nullptr) {
	...
	if (p == nullptr || v == nullptr) {
		for (int i = 0; i < in->datas.Count(); ++i) {
			list[i].p = in->datas[i].pos;
			list[i].p = list[i].p - offsetP;
			list[i].v = in->datas[i].vel;
		}
	}
	else if (v != nullptr && p != nullptr) {
//#pragma omp parallel for
		//for (int i = 0; i < in->datas.Count(); ++i) {
		concurrency::parallel_for(size_t(0), size_t(in->datas.Count()), [&](int i){
			list[i].p = p[i];
			list[i].v = v[i];
		});
	...
	cudaDeviceSynchronize();
	cudaMemcpy(d_list, list, in->datas.Count() * sizeof(Particle<T>), cudaMemcpyHostToDevice);
...
//#pragma omp parallel for
	...
	concurrency::parallel_for(size_t(0), size_t(in->datas.Count()), [&](int i){
		...
		in->datas[i].vel.x = (list[i].p.x + offsetP.x - in->datas[i].pos.x) / in->dt;	//there is loss of precision here since in->datas stores positions as floats!
		in->datas[i].vel.y = (list[i].p.y + offsetP.y - in->datas[i].pos.y) / in->dt;
		in->datas[i].vel.z = (list[i].p.z + offsetP.z - in->datas[i].pos.z) / in->dt;
	});
}
...
__device__ void CuGrid<T>::advectParticles() {													//advect particles!
	...
	for (int i = threadIdx.x + blockDim.x*blockIdx.x; i < numParticles; i += offset) {
		Particle<T> k2 = list[i], k3;
		k2.p += k2.v*(dt / 2.0);
		a.interpUtoP(k2);
		k3 = k2;
		k3.p += k3.v*dt*(3.0 / 4.0);
		a.interpUtoP(k3);
		list[i].p += list[i].v*dt*(2.0 / 9.0) + k2.v*dt*(3.0 / 9.0) + k3.v*dt*(4.0 / 9.0);
	}
}
...
//#pragma omp parallel for
	...
	concurrency::parallel_for(size_t(0), size_t(numParticles), [&](int p){
		Particle<T> k2 = list[p], k3;
		k2.p += k2.v*(dt / 2.0);
		a.interpUtoP(k2);
		k3 = k2;
		k3.p += k3.v*dt*(3.0 / 4.0);
		a.interpUtoP(k3);
		list[p].p += dt*(list[p].v*(2.0 / 9.0) + k2.v*(3.0 / 9.0) + k3.v*(4.0 / 9.0));
	});
}
...
__device__ void CuGrid<T>::reInsert(curandState_t* states) {
	...
	for (int i = threadIdx.x + blockDim.x*blockIdx.x; i < numParticles; i += offset) {
		if (a.get(floor(list[i].p.x / dx), floor(list[i].p.y / dx), floor(list[i].p.z / dx)).t == SOLID) {
			a.insertToRandomPureFluid(list[i], states);
		}
	}
}
...
//#pragma omp parallel for
	//for (int p = 0; p < numParticles; ++p) {
	concurrency::parallel_for(size_t(0), size_t(numParticles), [&](int p){
		int i = floor(list[p].p.x / dx), j = floor(list[p].p.y / dx), k = floor(list[p].p.z / dx);
		if (a.get(i, j, k).t == SOLID) {
			int i = a.pFluidIndexes[(int)(a.dist->operator()(*(a.gen))*a.numPFluids)];
			int tz = i / (x*y), ty = (i % (x*y)) / x, tx = (i % (x*y)) % x;
			list[p].p.x = tx * dx + a.dist->operator()(*(a.gen))*dx;
			list[p].p.y = ty * dx + a.dist->operator()(*(a.gen))*dx;
			list[p].p.z = tz * dx + a.dist->operator()(*(a.gen))*dx;
			a.interpUtoP(list[p]);
		}
	});
}
...
__device__ void CuGrid<T>::construct() {
	int offset = gridDim.x*blockDim.x;
	for (int i = threadIdx.x + blockDim.x*blockIdx.x; i < numParticles; i += offset) {
		int tx = floor(list[i].p.x / dx), ty = floor(list[i].p.y / dx), tz = floor(list[i].p.z / dx);
		...
		for (int ioff = -1; ioff < 2; ++ioff) {
			for (int joff = -1; joff < 2; ++joff) {
				for (int koff = -1; koff < 2; ++koff) {
					t = a.kWeight(list[p].p - Vec3<T>((i + ioff + 1)*dx, (j + joff + 0.5)*dx, (k + koff + 0.5)*dx));
					a.get(i + ioff, j + joff, k + koff).sumk.x += t;
					a.get(i + ioff, j + joff, k + koff).sumuk.x += list[p].v.x*t;

					t = a.kWeight(list[p].p - Vec3<T>((i + ioff + 0.5)*dx, (j + joff + 1)*dx, (k + koff + 0.5)*dx));
					a.get(i + ioff, j + joff, k + koff).sumk.y += t;
					a.get(i + ioff, j + joff, k + koff).sumuk.y += list[p].v.y*t;

					t = a.kWeight(list[p].p - Vec3<T>((i + ioff + 0.5)*dx, (j + joff + 0.5)*dx, (k + koff + 1)*dx));
					a.get(i + ioff, j + joff, k + koff).sumk.z += t;
					a.get(i + ioff, j + joff, k + koff).sumuk.z += list[p].v.z*t;
				}
			}
		}
	}
...
void CuGrid<T>::maxUCPU() {
	T max = 0;
	for (int p = 0; p < numParticles; ++p) {
		if (abs(list[p].v.x) > max)
			max = abs(list[p].v.x);
		if (abs(list[p].v.y) > max)
			max = abs(list[p].v.y);
		if (abs(list[p].v.z) > max)
			max = abs(list[p].v.z);
	}
	a.mU = max;
}
